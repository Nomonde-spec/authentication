...
export function Sidebar({ activeSection, onSectionChange, isOpen, onClose, mobile = false }: SidebarProps) {
  const renderSidebarContent = () => (
    <>
      ...
      <div className="mt-8 border-t border-white/10 pt-6 text-slate-500">
        ...
        {otherItems.map((item) => {
          ...
          return (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-[20px] px-4 py-3 text-left text-sm text-slate-400 transition duration-300 hover:bg-white/5 hover:text-white"
            >
              <ItemIcon size={18} />
              {item.label}
            </button>
      ...
      <div className="mt-auto rounded-[24px] border border-white/10 bg-[#111827]/80 p-4 shadow-[0_20px_60px_-35px_rgba(0,0,0,0.35)]">
        ...
        <button className="mt-4 flex w-full items-center justify-between rounded-[18px] border border-white/10 bg-[#111827] px-4 py-3 text-sm text-slate-200 transition duration-300 hover:bg-white/5">
          <span>Logout</span>
          <LogOut size={18} />
        </button>
      </div>
  ...
  if (mobile) {
    return (
      <motion.aside
        ...
        className="fixed left-0 top-0 z-50 h-full w-4/5 max-w-[280px] overflow-y-auto rounded-r-[28px] border-r border-white/10 bg-[#111827]/95 p-6 shadow-[0_30px_120px_-80px_rgba(0,0,0,0.5)] backdrop-blur-xl lg:hidden"
      >
